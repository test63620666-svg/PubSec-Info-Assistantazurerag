...
echo -e "\n"
sudo DOCKER_API_VERSION=1.43 docker build -f ./container_images/enrichment_container_image/Dockerfile -t enrichmentapp . --build-arg BUILDKIT_INLINE_CACHE=1

# Generate a unique tag for the image
tag=$(date -u +"%Y%m%d-%H%M%S")
echo "Tagging image with: $tag"
sudo DOCKER_API_VERSION=1.43 docker tag enrichmentapp enrichmentapp:$tag
